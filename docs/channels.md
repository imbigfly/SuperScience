...
Settings → Channel Access connects IM bots to the workspace agent: messages you send
...
callback URL is needed. The recommended setup is **Settings → Channel Access →
...
SuperScience stores the returned App Secret directly in the local secrets file; the device code
...
4. Paste App ID / App Secret in Settings → Channel Access, select the matching
   region, save, then toggle on. The secret is stored in the local secrets file.
...
bind** in Settings → Channel Access and confirm in WeChat. The scanning account
becomes the owner — only its 1:1 messages are handled; group messages are
ignored. The bot token lives in the local secrets file; unbind removes it.
