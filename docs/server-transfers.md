...
SuperScience transfers one exact file or directory from a registered, probed SSH
...
- `relay` downloads into a private temporary directory on the SuperScience machine and
...
SuperScience authenticates through the selected source context once and downloads with
...
With `action=install`, SuperScience:
...
With `action=verify`, SuperScience verifies and records trust the user configured
