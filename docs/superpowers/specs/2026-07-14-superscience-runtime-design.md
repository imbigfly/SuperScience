# SuperScience Runtime: Persistent Local/Remote Python and R Sessions

**Date:** 2026-07-14
**Status:** Implemented (PR 1–5)
**Scope:** Rename `superscience-python` to `superscience-runtime` and evolve it into the
project-level manager for persistent exploratory Python and R sessions running in
local, WSL, or SSH execution contexts.

## 1. Decision summary

superscience needs two different execution planes:
...
- `crates/superscience-python` provisions `app_data/python/.venv` with `uv`, launches
...
but MCP process ownership remains in `superscience-mcp`/the host wiring and never moves into
`RuntimeManager`.
...
## 8. `superscience-runtime` crate boundary

`crates/superscience-python` is renamed to `crates/superscience-runtime`; keeping both crates would
create an unnecessary compatibility layer.

`superscience-runtime` owns:
...
directory or changing the `SUPERSCIENCE_KERNEL_WORKER` development override. R can add its
...
2. Desktop/CLI/MCP bridge wiring passes it to `superscience-mcp` for bundled bio-tools.
...
context parsing inside `superscience-runtime`. The CLI supplies the equivalent launcher for
...
| SSH | Existing `SshConnection` options + remote interpreter/worker command | Versioned path under `~/.superscience/runtime/` | SSH connection/app process |
...
- Rename `superscience-python` to `superscience-runtime` and update workspace dependencies, docs,
...
- Preserve `app_data/python`, the bundled `python/` resource path, and
  `SUPERSCIENCE_KERNEL_WORKER`; a crate rename is not a resource/data migration.
