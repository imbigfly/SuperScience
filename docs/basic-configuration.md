...
按 `Cmd/Ctrl+P` 打开操作面板并选择“隐私模式”，或直接按 `Cmd/Ctrl+Shift+H` 打开隐私模式弹窗。勾选不希望出现在录屏中的项目后点“一键隐藏”，首页会同时隐藏对应的项目卡片、最近会话和搜索结果，且不会显示隐私模式提示；需要恢复时再次打开弹窗并点“一键恢复”。项目选择和当前隐藏状态只保存在本机。
