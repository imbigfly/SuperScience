...
在输入框左下角 Agent 菜单的 **计算环境** 子菜单中，点击某个环境行右侧的星标即可把它设为默认分析环境（行内出现“默认”徽章）；再次点击星标取消。默认环境跨会话持久化：之后 Agent 调用 `python`、`r` 等接受 `context_id` 的工具时，省略 `context_id` 会在默认环境中运行而不是本机，并且默认环境会在首次使用时自动加入当前会话的已选环境。Agent 仍可通过显式传 `local` 使用本机。未设置默认（或默认环境已被删除）时行为不变，回落到本机。
...
当工具返回 MCP App 时，Wisp 会在中间区域打开应用标签页，并保留与聊天的分屏。同一 UI 资源（或没有 URI 时的同一工具名）再次打开或搜索时会复用已有标签并更新内容，而不是再堆一个窗口。在该 App 仍绑定原始 MCP 连接时，界面内的分页、预览等操作可以通过 `tools/call` 直接打到同一个 Server，不必新开一轮对话。重新打开已保存的会话时，如果原连接已经不在，App 会退回把选择交给下一轮模型。MCP App 也可以把最多 64 KiB 的文本/结构化选择状态加入下一轮模型上下文；关闭 App 会清除该状态。
