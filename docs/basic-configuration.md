...
- `/files`：打开右侧文件面板。
- `/upload`：打开文件上传对话框。
- `/share`：打开「分享为长图」：勾选消息、打码关键词，导出 PNG 或 HTML。点「用技能写小红书」会附加 `xiaohongshu-note` 技能，按勾选内容写文案。微信 / Twitter 文案仍可在「写社媒文案」里展开。顶栏铃铛左侧的分享按钮、输入框左侧 `+` 菜单的「分享」是同一个入口。会话还没有可分享的消息时，顶栏按钮会禁用。
