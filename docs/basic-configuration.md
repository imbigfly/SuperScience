...
当工具返回 MCP App 时，Wisp 会在中间区域打开应用标签页，并保留与聊天的分屏。在该 App 仍绑定原始 MCP 连接时，界面内的分页、预览等操作可以通过 `tools/call` 直接打到同一个 Server，不必新开一轮对话。重新打开已保存的会话时，如果原连接已经不在，App 会退回把选择交给下一轮模型。MCP App 也可以把最多 64 KiB 的文本/结构化选择状态加入下一轮模型上下文；关闭 App 会清除该状态。
