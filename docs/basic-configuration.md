...
- `/files`：打开右侧文件面板。
- `/upload`：打开文件上传对话框。
- `/share`：打开后会立刻准备好可粘贴的社媒文案和几张关键截图，不用自己写字或再截一张。默认按当前语言选小红书或 Twitter，换平台会重写文案。点「一键复制去发」即可贴到小红书、微信或 Twitter（有的 App 会分两次粘贴文字和图片）。需要打码、改选消息或导出整段长图 / HTML 时，展开「调整内容或导出长图」。输入框左侧 `+` 菜单的「分享」是同一个入口。
