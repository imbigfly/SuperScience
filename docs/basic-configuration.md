...
只有点击确认后草稿才会持久化。用户在消息中明确说“记住”“我的偏好”或 `remember` 时，任务完成后也会自动打开同一个确认窗口，仍不会自动保存。保存为全局记忆时可以选择新增，或显式选择一条旧记忆进行替换；Wisp 不会自行猜测两条记忆是否冲突。可以在 **设置 → 记忆** 查看或编辑项目记忆，并查看、编辑或删除全局习惯；关闭记忆后不会生成新草稿，也不会向 Agent 注入已有记忆。
