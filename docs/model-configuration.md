...
or the setting can be turned off to skip the extra model call entirely. SuperScience
...
`/chat/completions`, `/responses`, or `/v1/messages`; SuperScience adds the matching
...
optional **Endpoint suffix**. SuperScience joins the suffix to the Base URL first, then
adds the selected protocol's request path. For OpenAI-compatible services, SuperScience
...
`https://api.deepseek.com/anthropic` before SuperScience adds `/v1/messages`.
...
SuperScience also treats an SSE `error` payload and a Responses API status other than
...
The desktop app stores model profile metadata in the SuperScience app-data SQLite store (`superscience.sqlite`). Existing single-model installs are migrated into a `default` model profile the first time settings are loaded.

## Headless CLI

The `superscience` headless CLI uses environment variables and supports the
same API protocols:
