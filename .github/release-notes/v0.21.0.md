...
- **原生与 Claude 插件兼容**：支持 `.superscience-plugin/plugin.json`，也会在安装时规范化 Claude 插件布局（`.claude-plugin/plugin.json`、`.mcp.json` 与 `skills/*/SKILL.md`）。`${SUPERSCIENCE_PLUGIN_ROOT}` 和 `${CLAUDE_PLUGIN_ROOT}` 都会解析到不可变安装目录。
...
- **交互式 MCP Apps**：工具返回 MCP App 时，SuperScience 会在中央标签页打开隔离工作台，并保留聊天/工作台分屏；切回对话不会重载应用，关闭标签会释放应用。
- **工作台随会话恢复**：最新的 MCP App presentation 会随会话持久化；切换会话或重启 SuperScience 后重新进入对话，会自动恢复对应工作台和工具数据。
...
- **模型兼容性**：对 Codex/网关请求将内部 `python` 工具映射为 `superscience_python`，规避保留名称冲突；兼容匿名 relay tool call，并修正 BigModel Anthropic usage 统计。
...
- MCP Apps 暂不允许应用主动调用工具、打开外链、下载、提交表单或使用摄像头、麦克风和定位；HTML resource 会落到项目 `.superscience/plugin-artifacts/` 后在沙箱预览中打开。
...
**Full Changelog:** https://github.com/imbigfly/SuperScience/compare/v0.20.0...v0.21.0
