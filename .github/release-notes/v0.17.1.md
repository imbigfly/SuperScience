...
- **规划边界仍由 SuperScience 控制**：模型只能返回白名单模板 ID；工具、权限、后端、预算、依赖和 Reviewer 均由 SuperScience 构造并再次校验，不能由模型扩权或嵌套委派。
...
**English:** superscience v0.17.1 gives Manual, Assisted, and Automatic Agent delegation distinct real semantics, adds risk-gated automatic execution, and fixes recent panel, scrolling, clipping, and draft-control regressions.
...
- SuperScience continues to construct and validate all permissions, tools, backends, budgets, dependencies, and Reviewer steps from a fixed template whitelist.
...
**Full Changelog:** https://github.com/imbigfly/SuperScience/compare/v0.17.0...v0.17.1
