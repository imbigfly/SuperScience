# SuperScience v0.24.0

这是继 `v0.23.0` 之后的研究工作台体验与可靠性版本。SuperScience 现在能通过真实浏览器捕获可供视觉模型查看的当前页面截图；研究图谱、数据库中登记的 Artifact 和 Run 产物不再只存在于后台，均可直接在界面中查看、打开和追溯。此版本还加入 SSH 定向信任边的可见与撤销管理、存储空间与会话 token 用量概览，并改善长对话、文献预览、运行时初始化和 SSH 诊断。
...
- **控制本地 trace 占用**：`.superscience/subagents/` 仅保留最新 20 个 explore trace，避免长期项目无限增长。
...
- SSH trust edge 的撤销立即移除 SuperScience 的直接传输授权；如果任一端无法连接，远端受管公钥的清理会在界面中明确标记为失败，而不会恢复授权。
...
**Compatibility:** Installs over `v0.23.0` in place with no manual migration. `web_screenshot` works only through the existing real-browser bridge on HTTP(S) tabs and follows the same approval model. Revoking a trust edge immediately removes SuperScience authorization even when a disconnected host prevents remote-key cleanup. Remote file previews remain capped at 32 MB.

**Full Changelog:** https://github.com/imbigfly/SuperScience/compare/v0.23.0...v0.24.0
