...
发送后，执行环境和运行时引用会在用户消息中继续显示为带服务器或终端图标的
卡片；用于 Agent 上下文的内部标记不会作为纯文字出现在消息气泡中。
...
导出或删除会话。点击会话列表上方的“选择”，可多选会话并将它们一起移动到
同一分组或批量删除。远程文件行也提供可见的下载操作，同时仍可使用右键菜单。
...
对话形式打开（含完整工具/run 操作记录）。打开上游/下游示例时会把对应的
`assets_*.tar.gz` 解压到工作区，因此右侧面板可以正确预览 counts 与富集结果文件。
