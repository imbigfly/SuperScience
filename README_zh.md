...
- Ctrl+K / Ctrl+P 命令面板、会话文件夹、跨项目的代码与图表全局库，以及应用内
  ...
  消息原文片段。
- **反馈**打开独立支持弹窗（不占用 AI 对话窗口）：展示微信群二维码占位，
  并可直接编辑内容发邮件给客服。发送时自动附加版本、平台、模型与启动耗时等
  非敏感诊断信息。非机密 SMTP 配置见 `src-tauri/config/feedback.toml`；密码请
  复制 `feedback.local.toml.example` 为 gitignored 的 `feedback.local.toml`
  后再填写。
