...
impl Tool for PrepareMethodSearchTool {
    ...
    fn schema(&self) -> superscience_llm::ToolSchema {
        superscience_llm::ToolSchema::new(
            self.name(),
...
fn project_file(root: &Path, value: &str) -> Result<(PathBuf, String), String> {
    ...
    let resolved = superscience_tools::safety::validate_file_path(root, &input)?;
    if !resolved.is_file() {
        return Err(format!(
...
) -> Result<superscience_core::method_search::EvaluatorResult, String> {
...
pub(crate) async fn prepare_method_search_with_evaluator(
    ...
    ensure_bounded_file(
        &target_path,
        "target source",
        superscience_core::method_search::MAX_CANDIDATE_SOURCE_BYTES as u64,
    )?;
    ...
    for (reference_name, dependency_id) in dependencies {
        store
            .save_artifact_dependency(
                ...
                &dependency_id,
                Some(&reference_name),
                superscience_store::LineageBasis::Declared,
...
pub(crate) struct MethodSearchRunDetails {
    pub run: superscience_store::RunRecord,
    pub state: superscience_store::MethodSearchRunState,
    pub spec: MethodSearchSpec,
    pub audit: MethodSearchAuditReport,
    pub audit_artifact_version_id: String,
    pub candidates: Vec<superscience_store::MethodCandidate>,
    pub strategies: Vec<superscience_store::MethodStrategyStat>,
    pub outputs: Vec<superscience_store::RunOutput>,
    pub activity: Option<superscience_store::AgentWorkflowRunActivity>,
}
...
pub(crate) async fn cancel_method_search(
    ...
    if matches!(
        details.run.status,
        superscience_store::RunStatus::Draft | superscience_store::RunStatus::Paused
...
mod tests {
    ...
    async fn preparation_rejects_oversized_target_before_snapshot_or_evaluation() {
        ...
        std::fs::OpenOptions::new()
            ...
            .open(&target)
            .unwrap()
            .set_len((superscience_core::method_search::MAX_CANDIDATE_SOURCE_BYTES + 1) as u64)
            .unwrap();
