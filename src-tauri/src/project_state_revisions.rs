...
pub(crate) async fn record_completed_mainline_turn(
    ...
    if last.role != superscience_llm::Role::Assistant || !last.tool_calls.is_empty() {
        return Ok(None);
    }
...
mod tests {
    ...
    async fn append_turn(store: &Store, index: i64, question: &str, answer: &str) {
        ...
        store
            .append_message("main", message_seq, &superscience_llm::Message::user(question))
            .await
            .unwrap();
        store
            .append_message(
    ...
    async fn historical_exploration_restores_second_turn_after_compaction() {
        ...
        store
            .replace_messages(
                "main",
                &[
                    superscience_llm::Message::user("compacted context"),
                    superscience_llm::Message::assistant("third answer"),
                ],
            )
            .await
            .unwrap();
