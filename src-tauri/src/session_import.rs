//! Import a superscience session-export archive (produced by `session_export::export_session`)
...
fn read_zip_string<R: Read + std::io::Seek>(
    ...
    let mut entry = zip
        .by_name(name)
        .map_err(|_| format!("not a superscience session export: {name} missing"))?;
...
fn parse_import_archive(path: &Path) -> Result<ParsedImport, String> {
    ...
    if manifest.session_id.trim().is_empty() {
        return Err("not a superscience session export: manifest session_id is empty".into());
    }
    ...
    if messages.is_empty() {
        return Err("not a superscience session export: no messages".into());
    }
...
async fn import_parsed(
        ...
        .map_err(|e| e.to_string())?
    {
        let stored = store
            ...
            .await
            .map_err(|e| e.to_string())?;
        // Only fast-forward: if the imported session was continued inside SuperScience
...
pub(super) async fn import_session_archive(
    ...
    app.dialog()
        .file()
        .add_filter("SuperScience session export", &["zip"])
        .pick_file(move |p| {
            let _ = tx.send(p);
        });
...
mod tests {
    ...
    fn parse_rejects_non_export_zip() {
        ...
        assert!(err.contains("not a superscience session export"), "{err}");
