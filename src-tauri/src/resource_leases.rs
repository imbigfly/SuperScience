...
pub(crate) enum AcquireResult {
    Acquired(superscience_tools::ToolResourceLease),
...
impl ProjectResourceCoordinator {
    pub(crate) fn try_acquire(
        ...
        AcquireResult::Acquired(superscience_tools::ToolResourceLease::new(move || {
            inner.release(id)
        }))
    }
}
...
fn normalized_project_path(root: &Path, raw: &str) -> Option<String> {
    let real = superscience_tools::safety::validate_file_path(root, raw).ok()?;
