...
fn latest_native_turn_is_complete(messages: &[superscience_llm::Message]) -> bool {
    ...
    messages[turn_start + 1..].iter().any(|message| {
        (message.role == superscience_llm::Role::Assistant && message.tool_calls.is_empty())
...
struct CheckpointSource {
    ...
    messages: Vec<superscience_llm::Message>,
}
...
pub(crate) fn write_context_archive(
    ...
    frame_id: &str,
    message_head: i64,
    messages: &[superscience_llm::Message],
) -> Result<(), String> {
...
mod tests {
    ...
    impl superscience_tools::ToolEnv for DenyExternalRunEnv {
    ...
    async fn repeated_start_reuses_one_checkpoint_for_independent_explorations() {
        ...
        let baseline_artifact_id = superscience_store::logical_artifact_id("p", logical_key);
        let baseline_version_id = service
            ...
            .await
            .unwrap();
        let baseline_resource = superscience_store::ExternalResource {
        ...
        service
            ...
            .replace_message_resource_links(
                "main",
                1,
                &[superscience_store::MessageResourceLink {
        ...
        let denied = superscience_tools::Tool::run(
            &external_tool,
            &serde_json::json!({
                "context_id": "ssh:gpu",
                "command": "echo branch"
        ...
        assert!(service
            ...
            .unwrap()
            .is_some());
        let private_resource = superscience_store::ExternalResource {
        ...
        assert!(service
            ...
            .is_empty());
        let mut second_run =
            superscience_store::RunRecord::new("second-run", "p", "local", "Second Run", "command");
        ...
        let decision = superscience_store::ResearchNode::new(
            "branch-decision",
            "p",
            superscience_store::ResearchNodeKind::Decision,
    ...
    async fn checkpoint_accepts_completion_tool_head_and_rejects_active_run() {
        ...
        service
            .store
            .append_message("main", 3, &superscience_llm::Message::user("unfinished"))
            .await
            .unwrap();
        assert!(service
        ...
        let mut run = superscience_store::RunRecord::new("run", "p", "local", "Run", "command");
