...
pub(crate) struct CreateWorkflowTool {
    store: Store,
    skills: Arc<superscience_skills::SkillIndex>,
}

impl CreateWorkflowTool {
    pub(crate) fn new(store: Store, skills: Arc<superscience_skills::SkillIndex>) -> Self {
        Self { store, skills }
    }
}
    ...
    for (key, value) in params {
        match key.as_str() {
            ...
            "capabilities" => {
                ...
                let registry = superscience_core::CapabilityRegistry::builtins();
...
mod tests {
    ...
    fn demo_skill_index() -> (Arc<superscience_skills::SkillIndex>, std::path::PathBuf) {
        let root =
        ...
        (
            Arc::new(superscience_skills::SkillIndex::load(&[root.clone()])),
            root,
        )
    }
