...
impl StoreWorkflowRunActivityDriver {
    ...
    ) -> Result<superscience_core::AgentDelegationResponse, String> {
        request
            .activity
            .validate()
...
#[async_trait]
impl superscience_core::WorkflowRunActivityDriver for StoreWorkflowRunActivityDriver {
    async fn execute(
        &self,
        request: superscience_core::WorkflowRunActivityRequest,
    ) -> anyhow::Result<superscience_core::AgentDelegationResponse> {
...
pub(crate) struct ProviderCandidateGenerator {
    provider: Arc<dyn superscience_llm::Provider>,
}
...
impl ProviderCandidateGenerator {
    pub(crate) async fn from_profile(store: &Store, profile_id: &str) -> Result<Self, String> {
        ...
        Ok(Self {
            provider: Arc::from(superscience_llm::build(config)),
        })
    }
}
        ...
        let completion = tokio::time::timeout(
            ...
            self.provider.complete(
                &[
                    ...
                    superscience_llm::Message::user(payload),
                ],
                &[],
            ),
        )
...
fn exact_artifact_path(root: &Path, context: &ArtifactVersionContext) -> Result<PathBuf, String> {
    ...
    let path = superscience_tools::safety::validate_file_path(root, &context.version.storage_path)?;
    if !path.is_file() {
        return Err(format!(
...
fn read_candidate_blob(project_root: &Path, blob: &MethodCandidateBlob) -> Result<Vec<u8>, String> {
    let path = superscience_tools::safety::validate_file_path(project_root, &blob.storage_path)?;
    let bytes = std::fs::read(path).map_err(|error| error.to_string())?;
...
async fn validate_method_search_activation(
    ...
    let (current_context, _) = superscience_store::canonical_json_sha256(
    ...
    let (stored_context, _) = superscience_store::canonical_json_sha256(&stored_context);
...
pub(crate) async fn terminal_activity_response(
    ...
    let status = match run.status {
        RunStatus::Succeeded => superscience_core::DelegationStatus::Succeeded,
        RunStatus::Cancelled => superscience_core::DelegationStatus::Cancelled,
        _ => superscience_core::DelegationStatus::Failed,
    };
    Ok(superscience_core::AgentDelegationResponse {
        ...
        child_frame_id: None,
        error: (status == superscience_core::DelegationStatus::Failed).then(|| {
            run.last_poll_error
...
pub(crate) async fn settle_linked_workflow_attempt(
    ...
    attempt.status = match response.status {
        superscience_core::DelegationStatus::Succeeded => AgentWorkflowAttemptStatus::Succeeded,
        superscience_core::DelegationStatus::Cancelled => AgentWorkflowAttemptStatus::Cancelled,
    ...
    async fn fixture_with_budget(
        ...
        let prepared = prepare_method_search_with_evaluator(
            ...
            PrepareMethodSearchRequest {
                ...
                direction: superscience_core::method_search::ScoreDirection::Maximize,
                guardrails: vec![superscience_core::method_search::MethodSearchGuardrail {
                    metric: "runtime_seconds".into(),
                    op: superscience_core::method_search::GuardrailOperator::Lte,
                    value: 10.0,
                }],
