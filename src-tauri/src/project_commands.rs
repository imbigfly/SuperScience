...
pub(super) const APP_WINDOW_TITLE: &str = superscience_paths::PRODUCT_NAME;
...
pub(super) async fn spawn_project_window(
    ...
    let mut builder = tauri::WebviewWindowBuilder::new(app, &label, url)
        .title({
            let product = superscience_paths::product_display_name(&locale);
            match name.trim() {
                "" => product.to_string(),
                n => format!("{product} \u{2014} {n}"),
            }
        })
