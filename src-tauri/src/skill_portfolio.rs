...
struct PlanningSkill {
    ...
    tags: Vec<String>,
    scope: String,
    metadata: Option<SuperscienceSkillMetadata>,
}
...
async fn planner_provider(
    ...
    model_id: &str,
    host: &superscience_core::DelegationHostPolicy,
    ...
    Ok((superscience_llm::build(config), profile.label))
}
...
mod tests {
    ...
    fn skill(id: &str, side_effects: SkillSideEffects) -> PlanningSkill {
        PlanningSkill {
            ...
            tags: vec![],
            scope: "bundled".into(),
            metadata: Some(SuperscienceSkillMetadata {
                schema_version: 1,
                domains: vec![],
                research_stages: vec![],
                roles: vec![],
