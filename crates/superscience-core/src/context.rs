...
use std::path::Path;
use superscience_llm::{Content, Message, Part, Provider, Role, ToolCall};
...
impl ContextManager {
    ...
    pub fn append_tool(
        ...
        content: superscience_llm::Content,
    ) {
            ...
            for mut m in turn.clone() {
                ...
                if m.role == Role::Tool {
                    ...
                    if !content.is_empty() && !content.starts_with(TOMBSTONE_PREFIX) {
                        m.content = superscience_llm::Content::text(tombstone);
                    }
                } else if m.role == Role::Assistant {
                    let content = m.content.as_text();
                    if (content.len() / 4 + 4) > OLD_ASSISTANT_MAX_TOKENS {
                        m.content = superscience_llm::Content::text(Self::compact_text(
                            &content,
...
pub fn image_content(label: &str, data_url: &str) -> superscience_llm::Content {
    superscience_llm::Content::Parts(vec![
        superscience_llm::Part::Text {
            kind: "text".into(),
            text: label.into(),
        },
        superscience_llm::Part::Image {
            kind: "image_url".into(),
...
mod tests {
    ...
    use superscience_llm::{Completion, ToolSchema};
