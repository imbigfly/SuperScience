...
pub trait Output: Send + Sync {
    ...
    fn on_message(&self, _msg: &superscience_llm::Message) {}
...
/// Adapter exposing `Output` as a `superscience_tools::ToolEnv`.
...
impl<'a> superscience_tools::ToolEnv for ToolEnvAdapter<'a> {
...
/// Adapter exposing `Output` as a `superscience_llm::StreamSink` (text + reasoning
...
impl<'a> superscience_llm::StreamSink for StreamSinkAdapter<'a> {
    ...
    fn on_usage(&mut self, _u: superscience_llm::Usage) {}
    fn is_cancelled(&self) -> bool {
        self.cancel
            .is_some_and(|c| c.load(std::sync::atomic::Ordering::Relaxed))
    }
