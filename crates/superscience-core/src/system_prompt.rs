...
About the model that powers you: your provider and model are configured by the host (the superscience app) and chosen \
...
tell them the underlying model is whatever is set in superscience's Settings (provider + model), that you can't reliably \
...
mod tests {
    ...
    #[test]
    fn identity_names_superscience_and_stays_model_agnostic() {
        let skills = SkillIndex::default();
        ...
        assert!(
            out.contains("superscience's Settings"),
