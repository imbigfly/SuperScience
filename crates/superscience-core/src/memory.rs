...
//!
//! Notes append to a per-day file under `<root>/.superscience/memory`. Retrieval accepts
...
impl MemoryManager {
    ...
    /// Point at `<root>/.superscience/memory` without creating it. Use for read-only
    /// browsing of another project's notes; writers still create the directory.
    pub fn at(root: &Path) -> Self {
        Self {
            dir: root.join(".superscience").join("memory"),
        }
    }
...
mod tests {
    ...
    fn at_does_not_create_memory_dir() {
        ...
        assert_eq!(memory.dir(), root.join(".superscience").join("memory"));
