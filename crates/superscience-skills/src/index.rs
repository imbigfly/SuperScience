...
mod tests {
    ...
    fn parses_yaml_block_scalar_description() {
        ...
        let dir = std::env::temp_dir().join(format!(
            "superscience-skill-blockscalar-{}",
            std::process::id()
        ));
    ...
    #[test]
    fn self_awareness_skill_uses_the_real_superscience_tool_contract() {
        ...
        for tool in [
            ...
            assert!(
                ...
                "missing SuperScience tool: {tool}"
            );
        }
        for stale in [
            "host.",
    ...
    #[test]
    fn gpu_skills_use_the_persisted_superscience_run_lifecycle() {
        let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../skills");
        for entry in std::fs::read_dir(&root).expect("read bundled skills") {
            let path = entry
