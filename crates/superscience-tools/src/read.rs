...
mod tests {
    ...
    async fn rejects_binary_pdf() {
        let tmp =
            std::env::temp_dir().join(format!("superscience_read_pdf_{}", std::process::id()));
    ...
    async fn rejects_non_regular_files() {
        let tmp =
            std::env::temp_dir().join(format!("superscience_read_type_{}", std::process::id()));
        std::fs::remove_dir_all(&tmp).ok();
        std::fs::create_dir_all(&tmp).unwrap();
    ...
    async fn logical_history_references_resolve_inside_the_working_project() {
        ...
        let history = root.join(".superscience/history");
        std::fs::create_dir_all(&history).unwrap();
        std::fs::write(history.join("archive-1.json"), "archived context").unwrap();

        let result = ReadTool
            .run(
                &json!({ "path": "superscience-history:archive-1" }),
                &TestEnv(root.clone()),
            )
            .await;
        assert!(result.success, "{}", result.content);
        ...
        assert!(
            !ReadTool
                .run(
                    &json!({ "path": "superscience-history:../outside" }),
                    &TestEnv(root.clone()),
                )
                .await
                .success
