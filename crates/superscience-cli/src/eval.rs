...
async fn evaluate(cfg: ProviderConfig) -> Result<EvalReport> {
    ...
    Ok(EvalReport {
        ...
        superscience_version: env!("CARGO_PKG_VERSION").into(),
...
fn build_agent(cfg: ProviderConfig, root: &Path) -> (Agent, String) {
    ...
    agent.tools =
        superscience_core::build_registry(skills.clone(), memory, false).filtered(&allowed);
    ...
    fn visit(root: &Path, dir: &Path, files: &mut BTreeMap<String, Vec<u8>>) -> Result<()> {
        for entry in std::fs::read_dir(dir)? {
            ...
            if relative
                .components()
                .next()
                .is_some_and(|part| part.as_os_str() == ".superscience")
            {
                continue;
            }
...
impl TempWorkspace {
    fn new(id: &str) -> Result<Self> {
        ...
        let path = base.join(format!(
            "superscience-agent-eval-{}-{nonce}-{sequence}-{id}",
            std::process::id()
        ));
    ...
    fn drop(&mut self) {
        let is_eval_dir = self.path.parent() == Some(self.base.as_path())
            && self
                ...
                .is_some_and(|name| name.starts_with("superscience-agent-eval-"));
        if is_eval_dir {
...
mod tests {
    ...
    fn report(scenarios: Vec<ScenarioResult>) -> EvalReport {
        EvalReport {
            suite_version: SUITE_VERSION.into(),
            superscience_version: "test".into(),
