...
use superscience_core::{Agent, ContextManager, ExploreTool, GuidanceQueue, MemoryManager, Output};
...
struct EvalCase {
    ...
    /// memory setting. Seed notes with `files` under `.superscience/memory/*.md`.
    #[serde(default)]
    memory_enabled: bool,
...
impl ProviderSource {
    fn build(&self) -> Box<dyn Provider> {
        match self {
            ...
            Self::Live(config) => superscience_llm::build(config.clone()),
        }
    }
...
#[async_trait]
impl superscience_runtime::RuntimeLauncher for EvalRuntimeLauncher {
    async fn launch(
        &self,
        _key: &superscience_runtime::RuntimeKey,
        _cwd: &Path,
    ) -> Result<superscience_runtime::LaunchedRuntime> {
        Ok(superscience_runtime::LaunchedRuntime::new(
            Box::new(EvalRuntimeKernel::default()),
            superscience_runtime::RuntimeMetadata {
...
#[async_trait]
impl superscience_runtime::RuntimeKernel for EvalRuntimeKernel {
    async fn execute(
        &mut self,
        _id: &str,
        code: &str,
        _output: &superscience_runtime::RuntimeOutput,
    ) -> Result<superscience_runtime::KernelResp> {
        let stdout = if let Some(value) = code.strip_prefix("set:") {
            ...
            return Ok(superscience_runtime::KernelResp {
                error: Some(format!("unsupported fixture code '{code}'")),
                ..superscience_runtime::KernelResp::default()
            });
        };
        Ok(superscience_runtime::KernelResp {
            stdout,
            ..superscience_runtime::KernelResp::default()
        })
    }

    async fn inspect(&mut self, _id: &str) -> Result<superscience_runtime::RuntimeObjectList> {
        Ok(superscience_runtime::RuntimeObjectList {
            objects: vec![superscience_runtime::RuntimeObject {
                name: "value".into(),
...
fn build_agent(
    ...
    if case.fixture_runtimes {
        let manager = superscience_runtime::RuntimeManager::new(Arc::new(EvalRuntimeLauncher));
        let project_id = root.to_string_lossy().into_owned();
        registry.add(Box::new(superscience_runtime::ReplTool::new(
            manager.clone(),
            &project_id,
        )));
