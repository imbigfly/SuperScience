...
use superscience_core::{Agent, Output, OutputFuture};
use superscience_llm::{Message, ToolCall};
use superscience_tools::{Approval, ConfirmDecision};

pub const RPC_SCHEMA: &str = "superscience.agent-rpc.v1";
...
mod tests {
    ...
    fn parses_versioned_commands_and_rejects_unknown_schemas() {
        let command = parse_command(
            r#"{"schema":"superscience.agent-rpc.v1","id":"turn-1","type":"prompt","prompt":"inspect"}"#,
        )
        .unwrap();
