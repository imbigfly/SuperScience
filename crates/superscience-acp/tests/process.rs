...
fn main() -> ExitCode {
    ...
    match runtime.block_on(run_tests()) {
        Ok(()) => {
            println!("superscience-acp process tests passed");
            ExitCode::SUCCESS
        }
        Err(error) => {
            eprintln!("superscience-acp process tests failed: {error}");
            ExitCode::FAILURE
        }
    }
}
...
async fn test_environment_propagation() -> Result<(), String> {
    ...
    profile.env.insert(
        "SUPERSCIENCE_ACP_TEST_ENV".into(),
...
async fn test_stderr_bound_and_drop_cleanup() -> Result<(), String> {
    let marker = unique_temp_path("superscience-acp-cleanup");
...
fn fake_agent(args: &[String]) -> ExitCode {
    ...
    match scenario {
        ...
        "environment" => {
            if std::env::var("SUPERSCIENCE_ACP_TEST_ENV").as_deref() != Ok("controlled-child") {
