...
fn authorized(headers: &HeaderMap, state: &RelayHttpState) -> bool {
    headers
        ...
        .is_some_and(|token| {
            const MESSAGE: &[u8] = b"superscience-relay-bearer-token";
            let expected_key =
...
mod tests {
    ...
    async fn bearer_auth_is_required_for_relay_routes() {
        let root =
            std::env::temp_dir().join(format!("superscience-http-auth-{}", uuid::Uuid::new_v4()));
    ...
    async fn router_accepts_authenticated_blob_and_cas_commit_without_network() {
        let root =
            std::env::temp_dir().join(format!("superscience-http-route-{}", uuid::Uuid::new_v4()));
        let relay = FileRelay::open(&root).await.unwrap();
