//! `McpTool` — wraps a remote MCP tool as a `superscience_tools::Tool`.
...
use superscience_tools::{Approval, McpAppServer, Tool, ToolEnv, ToolEvent, ToolResult};
...
async fn materialize_html_resources(
    ...
    for block in &result.content {
        ...
        let relative = PathBuf::from(".superscience")
...
mod tests {
    ...
    async fn embedded_html_becomes_a_bounded_project_artifact() {
        ...
        assert!(paths[0].starts_with(".superscience/plugin-artifacts"));
