...
-- Snapshot bytes live under the project's .superscience/undo directory; SQLite keeps
...
-- messages and from SuperScience's built-in update_plan progress tool.  A frame may
...
-- Durable binding between a SuperScience frame and the session owned by an external
...
-- External coding-agent transcripts imported as SuperScience sessions (#464). The
-- legacy table/key names remain for compatibility; non-Codex ids are
-- provider-prefixed. Deleting the SuperScience session frees the id for import again.
