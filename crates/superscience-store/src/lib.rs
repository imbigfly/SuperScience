...
impl Store {
    ...
    async fn migrate(pool: &SqlitePool) -> Result<()> {
        sqlx::query(
            "CREATE TABLE IF NOT EXISTS superscience_schema_migrations (\
    ...
    async fn migration_applied(pool: &SqlitePool, version: &str) -> Result<bool> {
        let row: Option<(String,)> =
            sqlx::query_as("SELECT version FROM superscience_schema_migrations WHERE version=?")
                .bind(version)
                .fetch_optional(pool)
                .await?;
        Ok(row.is_some())
    ...
    async fn record_migration(pool: &SqlitePool, version: &str) -> Result<()> {
        sqlx::query(
            "INSERT OR IGNORE INTO superscience_schema_migrations(version,applied_at) VALUES(?,?)",
        )
        .bind(version)
        .bind(chrono::Utc::now().timestamp())
        .execute(pool)
    ...
    pub async fn schema_migrations(&self) -> Result<Vec<String>> {
        let rows: Vec<(String,)> = sqlx::query_as(
            "SELECT version FROM superscience_schema_migrations ORDER BY applied_at, version",
        )
        .fetch_all(&self.pool)
        .await?;
