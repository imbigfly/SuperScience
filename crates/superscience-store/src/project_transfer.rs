...
fn unavailable_path(raw: &str) -> String {
    let name = raw
        ...
        .collect::<String>();
    format!("superscience-unavailable://source-local/{name}")
}
...
impl Store {
    ...
    pub async fn export_project_database(
        ...
        for query in [
            ...
            "UPDATE superscience_schema_migrations SET applied_at=0",
        ] {
...
mod tests {
    ...
    fn windows_paths_become_portable_and_restore_on_macos() {
        ...
        assert_eq!(
            portable_project_path(root, r"file://C:\Users\Alice\SuperScience\Study\data\x.csv"),
            ("data/x.csv".into(), false)
        );
        assert_eq!(
            portable_project_path(root, "file:///C:/Users/Alice/SuperScience/Study/data/y.csv"),
            ("data/y.csv".into(), false)
        );
        let (outside, warned) = portable_project_path(root, r"D:\shared\large.fastq");
        assert!(warned);
        assert!(outside.starts_with("superscience-unavailable://"));
        assert_eq!(
    ...
    async fn project_database_roundtrip_rebinds_paths_and_stops_live_runs() {
        ...
        let source_path =
            std::env::temp_dir().join(format!("superscience_transfer_source_{token}.sqlite"));
        let archive_path =
            std::env::temp_dir().join(format!("superscience_transfer_archive_{token}.sqlite"));
        let target_path =
            std::env::temp_dir().join(format!("superscience_transfer_target_{token}.sqlite"));
        ...
        let workflow = source
            ...
            .unwrap();
        source
            .create_frame("frame-1", "project-1", "SUPERSCIENCE", "model")
            .await
            .unwrap();
        source
        ...
        let artifact_version_id = source
            .save_artifact_version(&ArtifactVersionDraft {
                ...
                storage_path: r"C:\Users\Alice\Study\.superscience\artifacts\sha256\ab\abcdef.png"
                    .into(),
        ...
        let input_version_id = source
            .save_artifact_version(&ArtifactVersionDraft {
                ...
                storage_path: r"C:\Users\Alice\Study\.superscience\artifacts\sha256\cd\counts.csv"
                    .into(),
        ...
        assert_eq!(
            ...
            "/Users/alice/Study/.superscience/artifacts/sha256/ab/abcdef.png"
        );
        assert_eq!(
            {
                let version = target
    ...
    async fn import_rejects_a_corrupt_frozen_publication_manifest() {
        let token = uuid::Uuid::new_v4();
        let source_path =
            std::env::temp_dir().join(format!("superscience_manifest_source_{token}.sqlite"));
        let archive_path =
            std::env::temp_dir().join(format!("superscience_manifest_archive_{token}.sqlite"));
        let target_path =
            std::env::temp_dir().join(format!("superscience_manifest_target_{token}.sqlite"));
        let source = Store::open(&source_path).await.unwrap();
        source
    ...
    async fn portable_hash_ignores_open_recency_but_detects_project_edits() {
        let token = uuid::Uuid::new_v4();
        let source_path =
            std::env::temp_dir().join(format!("superscience_hash_source_{token}.sqlite"));
        let first_path =
            std::env::temp_dir().join(format!("superscience_hash_first_{token}.sqlite"));
        let second_path =
            std::env::temp_dir().join(format!("superscience_hash_second_{token}.sqlite"));
        let edited_path =
            std::env::temp_dir().join(format!("superscience_hash_edited_{token}.sqlite"));
        let source = Store::open(&source_path).await.unwrap();
        source
