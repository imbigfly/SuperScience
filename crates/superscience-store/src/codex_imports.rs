//! Mapping between imported Codex CLI rollouts and SuperScience frames (#464).
...
mod tests {
    ...
    async fn store_with_frame() -> (Store, std::path::PathBuf) {
        let path = std::env::temp_dir().join(format!(
            "superscience_store_codex_imports_{}.sqlite",
            uuid::Uuid::new_v4()
        ));
    ...
    async fn codex_import_round_trips_and_cascades_on_delete() {
        ...
        // Deleting the SuperScience session frees the Codex id for re-import.
