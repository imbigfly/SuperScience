...
impl Store {
    ...
    /// markers on a freshly cloned frame. The corresponding `.superscience/history`
    ...
    pub async fn rewrite_cloned_context_archive_references(
        ...
        for row in rows {
            ...
            let updated = text
                .replace(&native_prefix, "superscience-history:")
                .replace(&slash_prefix, "superscience-history:");
            if updated == *text {
                continue;
            }
            *text = updated;
