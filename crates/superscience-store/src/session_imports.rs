//! Mapping between imported superscience session-export archives and SuperScience frames.
...
mod tests {
    ...
    async fn store_with_frame() -> (Store, std::path::PathBuf) {
        let path = std::env::temp_dir().join(format!(
            "superscience_store_session_imports_{}.sqlite",
            uuid::Uuid::new_v4()
        ));
    ...
    async fn session_import_round_trips_and_cascades_on_delete() {
        ...
        // Deleting the SuperScience session frees the source id for re-import.
