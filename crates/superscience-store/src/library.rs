...
mod tests {
    ...
    async fn legacy_databases_are_rebuilt_to_accept_text_items() {
        let path = std::env::temp_dir()
            .join(format!(
                "superscience-library-migrate-test-{}",
                uuid::Uuid::new_v4()
            ))
            .join("library.sqlite");
    ...
    async fn project_deletion_cannot_cascade_into_the_global_library() {
        let dir = std::env::temp_dir().join(format!(
            "superscience-library-separate-db-test-{}",
            uuid::Uuid::new_v4()
        ));
        let project_store = Store::open(&dir.join("superscience.sqlite")).await.unwrap();
        ...
        project_store
            ...
            .unwrap();
        project_store
            .create_frame("session-1", "project-1", "SUPERSCIENCE", "model")
            .await
            .unwrap();
        project_store
