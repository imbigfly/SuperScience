...
Severity is assigned by these decision-impact definitions alone (#574 A3/B1): adversarial register never inflates a band, politeness never deflates one, and the same defect class with the same decision impact lands in the same band every time. CRITICAL/MAJOR/MINOR map onto the Schema 6 `severity` enum (`critical`/`major`/`minor` — the single source, `../academic-shared/handoff_schemas.md` § Weakness Object); OBSERVATION is a non-defect channel and never enters `weaknesses[]`.
