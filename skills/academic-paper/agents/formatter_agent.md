...
`../academic-shared/contracts/submission/format_profile.schema.json`.
...
Validate against `../academic-shared/contracts/passport/bibliographic_integrity_signal.schema.json` and sort rows lexically by `signal_id`.
...
`terminal_policy.eligible: false` cannot trigger refusal or `HIGH-BLOCK`. A v1.1 eligible retraction row is still only an input to the finalizer; this formatter never evaluates it. Migration/deprecation authority is `../academic-shared/bibliographic_integrity_signals.md`.
