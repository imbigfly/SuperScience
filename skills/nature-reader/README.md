...
`nature-reader` 用于把论文 PDF、DOI、arXiv、出版社 HTML 或粘贴文本转换为可回溯的全文中英对照 HTML 报告，包含图表定位、可直接阅读的公式和 source map。
...
- 为每段文本、图、表建立页码和来源锚点。
- 在 HTML 报告中渲染公式，并为每个独立公式建立 `E...` 锚点；`paper.md` 仍用 `$...$` / `$$...$$` 供校验和下游技能。
...
- `reader.html`：给用户打开的中英对照报告（由 `scripts/build_reader_html.py` 生成）。
- `paper.md`：给下游技能和公式校验用的全文材料。
...
- 默认以 `reader.html` 为用户交付物，不默认生成交互式问答面板。
