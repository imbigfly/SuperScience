...
Use this skill to turn a research paper into a complete bilingual reading artifact. The user-facing default is a single `reader.html` report generated by `scripts/build_reader_html.py`. Keep `paper.md` and `source_map.json` for downstream skills. The report should read like a bilingual paper companion, not a summary dump:
...
- render equations with stable `E...` anchors (Markdown math in `paper.md`; KaTeX in the generated HTML); never expose bare LaTeX commands as ordinary prose
...
- write a complete `source_map.json`, `paper.md`, `translation_notes.md`, and `assets/`, then always generate `reader.html` with the bundled builder
...
If constraints prevent full processing, still create a draft `reader.html` and clearly label missing pages, missing figures/tables, untranslated blocks, or low-confidence OCR/crops in `translation_notes.md`.
...
For copyrighted publisher PDFs, keep chat responses short and point to the local `reader.html`. In local files, include the bilingual reader only for the user-provided source file or clearly lawful open-access content; avoid reproducing large copyrighted text directly in chat.
...
- read equations in the HTML report or inspect the original crop when transcription is uncertain
- move through a complete HTML report without losing source traceability
