...
- **Data terminology and governance planning**: Use `../academic-shared/references/irb_terminology_glossary.md` for terminology, then apply only actor/scope-matched registry requirements. Plan retention and destruction without treating a retained relink key or any one regime's terminology as universal.
...
> References: portable navigation in `references/irb_decision_tree.md`; authority-selection and consumer rules in `../academic-shared/references/human_subjects_authority_protocol.md`; bounded requirements in `../academic-shared/human_subjects_authority_registry.json`; resolved shape in `../academic-shared/contracts/human_subjects/resolved_authority_context.schema.json`; candidate rule-trace rules in `../academic-shared/references/review_pathway_rule_trace_protocol.md`; request/trace shapes in `../academic-shared/contracts/human_subjects/review_pathway_trace_request.schema.json` and `../academic-shared/contracts/human_subjects/review_pathway_rule_trace.schema.json`; deterministic packet rules in `../academic-shared/references/submission_packet_manifest_protocol.md`; manifest shape in `../academic-shared/contracts/human_subjects/submission_packet_manifest.schema.json`; advisory rules in `../academic-shared/references/authority_content_coverage_advisory_protocol.md`; advisory shape in `../academic-shared/contracts/human_subjects/content_coverage_advisory.schema.json` (schemas alone are not replay validation).
...
- If human subjects are involved, administrative planning is mandatory; `references/irb_decision_tree.md` is navigation only, and profile-dependent content must pass the replay-validated gate in `../academic-shared/references/human_subjects_authority_protocol.md`
...
- Consent / privacy language must pass through `../academic-shared/references/irb_terminology_glossary.md` before output. Anonymity, confidentiality, de-identification, and pseudonymization are not interchangeable.
- For every item labeled "reverse-coded": include a one-line construct-equivalence justification confirming same construct on same Likert dimension. True reverse vs contrast distinction is mandatory. See `../academic-shared/references/psychometric_terminology_glossary.md`.
