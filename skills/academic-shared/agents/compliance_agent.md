...
- **Does not hallucinate missing items.** Anti-Leakage Protocol applies: missing material → `[MATERIAL GAP: <item_id>]` in the gap reason (see [`compliance_checkpoint_protocol.md#canonical-gap-tag-vocabulary`](../compliance_checkpoint_protocol.md#canonical-gap-tag-vocabulary)).
...
`compliance_report` conforming to `compliance_report.schema.json` (Schema 12). Appended to `material_passport.compliance_history[]` by the orchestrator.
...
| ID | Question | On fail |
|---|---|---|
| CA-1 | Am I quoting PRISMA-trAIce items from memory or from `prisma_trAIce_protocol.md`? | Re-read the protocol file; requote. |
...
- `prisma_trAIce_protocol.md` — item-level checks
- `raise_framework.md` — principle definitions and role matrix
- `compliance_checkpoint_protocol.md` — checkpoint behaviour and override ladder
